...
// The checkout root — the Bun workspace root whose package.json declares
// "workspaces", NOT the runtime package dir (packages/runtime). Everything
// layout-relative hangs off this: the instance name above (worktree
// basename), the web app at packages/web, bundled skills/ and docs/ at the
// root. Found by walking up from this file to the nearest package.json with
// a "workspaces" field so the value survives the runtime package nesting
// deeper (a fixed hop count would silently re-root everything — and turn
// every instance name into "runtime"). Cached: callers hit this on every
// config load and the answer can't change within a process.
let cachedProjectRoot: string | undefined;

export function projectRoot(): string {
  cachedProjectRoot ??= findWorkspaceRoot(import.meta.dir);
...
function findWorkspaceRoot(startDir: string): string {
  ...
  while (true) {
    ...
    // Filesystem root reached without a workspace marker. Fall back to the
    // path the checkout root has in the packages/runtime/src layout.
