...
describe("web production bundle", () => {
  ...
  function markBuilt(runtimeDir: string, distDir: string): void {
    mkdirSync(join(runtimeDir, "packages", "web", distDir), { recursive: true });
    writeFileSync(join(runtimeDir, "packages", "web", distDir, "BUILD_ID"), "build-id\n");
  }
  ...
  test("builds into the sha-keyed dist dir via bun run build", async () => {
    ...
    expect(calls[0]!.cwd).toBe(join(runtimeDir, "packages", "web"));
  ...
  test("GCs other prod dist dirs on success, leaving the current bundle and dev dirs alone", async () => {
    ...
    mkdirSync(join(runtimeDir, "packages", "web", ".next-default"), { recursive: true });
    // Nor may the dev dist dir of an instance whose NAME starts with
    // "prod-": only sha-shaped (>=12 hex chars) suffixes are GC-able.
    mkdirSync(join(runtimeDir, "packages", "web", ".next-prod-foo"), { recursive: true });
    const { runStepImpl } = makeBuildRecorder();
    await buildWebProdBundle(runtimeDir, SHA, "pipe", { runStepImpl });
    expect(existsSync(join(runtimeDir, "packages", "web", `${WEB_PROD_DIST_PREFIX}${SHA}`))).toBe(true);
    expect(existsSync(join(runtimeDir, "packages", "web", `${WEB_PROD_DIST_PREFIX}0dd5a00000000`))).toBe(false);
    expect(existsSync(join(runtimeDir, "packages", "web", ".next-default"))).toBe(true);
    expect(existsSync(join(runtimeDir, "packages", "web", ".next-prod-foo"))).toBe(true);
  });
  ...
  test("preserves a named prod dir (the previous HEAD's bundle) while still GCing strictly-older ones", async () => {
    ...
    expect(existsSync(join(runtimeDir, "packages", "web", `${WEB_PROD_DIST_PREFIX}${SHA}`))).toBe(true);
    expect(existsSync(join(runtimeDir, "packages", "web", preserved))).toBe(true);
    expect(existsSync(join(runtimeDir, "packages", "web", older))).toBe(false);
    expect(existsSync(join(runtimeDir, "packages", "web", ".next-default"))).toBe(true);
  });
  ...
  test("a failed build throws (so updateRuntime aborts before scheduling a restart) and GCs nothing", async () => {
    ...
    await expect(buildWebProdBundle(runtimeDir, SHA, "pipe", { runStepImpl })).rejects.toThrow(/bun run build in packages\/web\/ failed/);
    // The old (still-servable-by-the-old-process) bundle survives a failure.
    expect(existsSync(join(runtimeDir, "packages", "web", `${WEB_PROD_DIST_PREFIX}0dd5a00000000`))).toBe(true);
  });
  ...
  function makeBuildRecorderWithFsSnapshot(runtimeDir: string, distDir: string) {
    ...
    const runStepImpl: RunStepImpl = async (_cmd, _args, _options) => {
      sawAtBuild["staleDevTypes"] = existsSync(join(runtimeDir, "packages", "web", ".next-default", "dev", "types"));
      sawAtBuild["staleBuildCache"] = existsSync(join(runtimeDir, "packages", "web", ".next-default", "cache"));
      sawAtBuild["servedServer"] = existsSync(join(runtimeDir, "packages", "web", ".next-default", "server"));
      sawAtBuild["servedStatic"] = existsSync(join(runtimeDir, "packages", "web", ".next-default", "static"));
  ...
  test("prunes stale generated route-types under every dist dir BEFORE building, leaving served bundles and build caches intact", async () => {
    ...
    mkdirSync(join(runtimeDir, "packages", "web", ".next-default", "dev", "types"), { recursive: true });
    writeFileSync(
      join(runtimeDir, "packages", "web", ".next-default", "dev", "types", "validator.ts"),
      "import '../../../src/app/threads/page.js';\n"
    );
    // Pure generated `types` under another dist dir must also go.
    mkdirSync(join(runtimeDir, "packages", "web", ".next", "types"), { recursive: true });
    // Non-types artifacts that must survive: a served bundle's runtime files
    // and the build cache (so incremental builds stay fast).
    mkdirSync(join(runtimeDir, "packages", "web", ".next-default", "cache"), { recursive: true });
    writeFileSync(join(runtimeDir, "packages", "web", ".next-default", "cache", "x"), "cache\n");
    mkdirSync(join(runtimeDir, "packages", "web", ".next-default", "server"), { recursive: true });
    writeFileSync(join(runtimeDir, "packages", "web", ".next-default", "server", "app.js"), "served\n");
    mkdirSync(join(runtimeDir, "packages", "web", ".next-default", "static"), { recursive: true });
    writeFileSync(join(runtimeDir, "packages", "web", ".next-default", "static", "chunk.js"), "served\n");
    ...
    expect(existsSync(join(runtimeDir, "packages", "web", ".next", "types"))).toBe(false);
    expect(existsSync(join(runtimeDir, "packages", "web", ".next-default", "dev", "types"))).toBe(false);
  ...
  test("does not prune (or build) when the sha dir already carries a BUILD_ID", async () => {
    ...
    expect(calls.length).toBe(0);
    expect(existsSync(join(runtimeDir, "packages", "web", ".next-default", "dev", "types"))).toBe(true);
  });
  ...
  test("resolveWebProdDistDir returns null without a BUILD_ID (aborted build must not be served)", () => {
    ...
    const sha12 = initRepoWithCommit(repo);
    mkdirSync(join(repo, "packages", "web", `${WEB_PROD_DIST_PREFIX}${sha12}`), { recursive: true });
...
describe("scheduleRuntimeRestart", () => {
  ...
  test("foreground (no supervisor, gateway not loaded): uses the detached bash stop+start helper", async () => {
    ...
    expect(script).toContain("packages/runtime/src/cli.ts stop --instance");
    expect(script).toContain("packages/runtime/src/cli.ts start --instance");
