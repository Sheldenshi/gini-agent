...
export async function createPairing(config: RuntimeConfig, input: Record<string, unknown>) {
  ...
  const created = await mutateState(config.lane, (state) => createPairingCode(state, ttlSeconds));
  return {
...
export async function claimPairing(config: RuntimeConfig, input: Record<string, unknown>) {
  ...
  const claimed = await mutateState(config.lane, (state) => claimPairingCode(state, code, deviceName));
  return {
...
export async function revokePairedDevice(config: RuntimeConfig, deviceId: string) {
  return redactDevice(await mutateState(config.lane, (state) => revokeDevice(state, deviceId)));
}
...
export async function authorizedBearer(config: RuntimeConfig, bearer: string | undefined): Promise<boolean> {
  ...
  const device = await mutateState(config.lane, (state) => findActiveDeviceByToken(state, bearer));
