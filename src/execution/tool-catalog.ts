...
const TOOL_DEFS: Array<ToolFunctionSpec & { toolset: string }> = [
  ...
  },
  {
    ...
    function: {
      name: "edit_soul",
      description: "Propose an edit to the active agent's SOUL.md (per-agent persona). Prefer `action: \"set\"` with the full consolidated SOUL.md body under H2 sections (`## Voice`, `## Style`, `## Boundaries`) — the current file is visible in the system prompt above, so emit the new version with the new rule integrated under the right section rather than appending below. Only call this when the user's CURRENT message asks for a NEW persona / voice / behavior rule for this agent. Write entries as facts about the agent's voice, not directives to yourself (\"Voice is terse\" ✓ — \"Always be terse\" ✗). Aim to keep the file under the soft cap shown in the SOUL persona header (1500 chars); when near or over cap, consolidate. The proposed body lands as SOUL.md.proposed and does NOT enter the system prompt until the user approves it via `POST /api/identity-files/soul/approve`. After calling, you MAY briefly mention the approval step in your reply but do NOT otherwise narrate the tool call. `action: \"append\"` adds a new section below the existing content (prefer set instead; append is a legacy fallback). `action: \"remove\"` drops the first paragraph containing the `needle` substring from the existing approved body; requires `needle`. Requires an active agent — there is no per-instance SOUL.",
      parameters: {
        type: "object",
        properties: {
          action: {
  ...
  },
  {
    ...
    function: {
      ...
      description: "Edit the instance-scoped USER.md (user profile). Prefer `action: \"set\"` with the full consolidated USER.md content under H2 sections (`## Identity`, `## Preferences`, `## Background`, `## Goals`) — the current file is visible in the system prompt above, so emit the new version with the new fact integrated under the right section rather than appending a chunk below. Only call this when the user's CURRENT message contains a NEW durable identity fact about themselves (name, role, location, employer, stable preference) that is NOT already in USER.md. Write entries as facts ABOUT the user, not directives to yourself (\"User prefers TypeScript\" ✓ — \"Always use TypeScript\" ✗); imperative phrasing in USER.md gets re-read next session as a system directive. Casual chat, follow-up questions, and topics unrelated to the user themselves are NOT identity facts — most turns should produce ZERO calls. Aim to keep the file under the soft cap shown in the USER profile header (1500 chars); when near or over cap, consolidate. DO NOT save task progress, PR/issue/commit IDs, completed-work logs, or other transient state — those belong in long-term memory (auto-retain handles them silently). Do NOT narrate the call in your reply: just acknowledge the fact briefly (\"Got it, X.\", \"Noted.\"). Auto-approved: writes go straight to USER.md and ride the system prompt on the next turn. USER.md is instance-scoped so the user's identity bridges across agent switches. The injection scan still gates content that trips a threat pattern. `action: \"append\"` adds a new section (legacy fallback; the storage layer de-duplicates lines that already exist); `action: \"remove\"` drops the first paragraph containing the `needle` substring (requires `needle`). Distinct from edit_soul which still requires user approval.",
      parameters: {
        type: "object",
        properties: {
          action: {
