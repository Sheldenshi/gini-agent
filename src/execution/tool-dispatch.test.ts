...
describe("request_connector dispatch", () => {
  ...
  test("substitutes `${var}` placeholders in the model's reason when params is passed", async () => {
    // The skill body owns the instruction text and tells the model to copy
    // the string verbatim, leaving `${project_id}` literal. The dispatcher
    // substitutes from `params` at call time so the URL ends up filled in.
    // The google-oauth-desktop provider does NOT declare a `requestInstructions`
    // template — substitution runs against the model's `reason` directly.
    const instance = `req-connector-substitute-${Math.random().toString(36).slice(2, 8)}`;
    const config = buildConfig(instance);
    const taskId = await newTask(config);
    const result = await dispatchToolCall(
      config,
      ...
      JSON.stringify({
        ...
        reason: "Open https://console.cloud.google.com/foo?project=${project_id}",
        params: { project_id: "abc-123" }
      })
    );
    expect(result.kind).toBe("pending");
    if (result.kind === "pending") {
      ...
      const approval = state.approvals.find((a) => a.id === result.approvalId);
      expect(approval).toBeDefined();
      // The placeholder is gone and the project id is substituted in.
      expect(approval!.reason).toBe("Open https://console.cloud.google.com/foo?project=abc-123");
      // payload.reason mirrors the substituted text for downstream consumers.
      ...
      expect(approval!.payload.templateParams).toEqual({ project_id: "abc-123" });
    }
  });

  test("leaves unknown `${var}` placeholders verbatim during substitution", async () => {
    // Better to surface a stray `${foo}` in the chat bubble than to silently
    // delete it — the user can flag the broken template and we can fix the
    // skill or provider.
    const instance = `req-connector-unknown-var-${Math.random().toString(36).slice(2, 8)}`;
    const config = buildConfig(instance);
    const taskId = await newTask(config);
    const result = await dispatchToolCall(
      config,
      ...
      JSON.stringify({
        provider: "google-oauth-desktop",
        reason: "Hello ${name}, your project is ${project_id}.",
    ...
    if (result.kind === "pending") {
      ...
      const approval = state.approvals.find((a) => a.id === result.approvalId);
      expect(approval).toBeDefined();
      expect(approval!.reason).toBe("Hello ${name}, your project is p-42.");
    }
  });

  test("passes the model's reason verbatim when neither template nor params is provided", async () => {
    // Default behavior — no provider template, no params from the model.
    // The model's `reason` becomes the approval's `reason` unmodified.
    const instance = `req-connector-verbatim-${Math.random().toString(36).slice(2, 8)}`;
    const config = buildConfig(instance);
    const taskId = await newTask(config);
    const result = await dispatchToolCall(
      config,
