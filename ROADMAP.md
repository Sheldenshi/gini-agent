...
- ⚪ **Zero anonymized telemetry by default.** No analytics, no "anonymized telemetry" sent automatically. If telemetry is ever added, it is opt-in with payload preview before the first send. Discoverable in code, not just policy. The one explicit, user-requested exception is the shipped crash reporting: it files a redacted GitHub issue only for instances the user has put under launchd supervision (opting in by enabling autostart), via the user's own `gh` CLI, to the user's own tracker — not anonymized phone-home telemetry to the maintainer. See [Crash Reporting And External Issue Filing](docs/adr/crash-reporting-and-issue-filing.md).
