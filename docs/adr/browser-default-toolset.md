...
- New browser actions that mutate user state but stay within the page (click, type, drag, select_option, tabs, scroll, navigate) inherit the action-argument-trail trust model. Do not add a `pendingOrAuto` wrapper to a new action of that shape. (`press` is deliberately omitted here — see Open Questions; it's currently classified low-risk and that may want revisiting before it joins this list.)
