...
turn. It is a **top-level, domain-agnostic primitive** (`packages/runtime/src/hooks/`), a sibling
...
`packages/runtime/src/capabilities/skill-script-hook.ts` (beside `skill-scripts.ts`; both core,
...
**Module shape.** The primitive lives in `packages/runtime/src/hooks/` and imports only `packages/runtime/src/types`
(for `RuntimeConfig`) and itself — zero `packages/runtime/src/jobs`, `packages/runtime/src/state`,
`packages/runtime/src/integrations`, or `packages/runtime/src/capabilities` imports, so it stays domain-free:
...
The `skill-script` handler lives in `packages/runtime/src/capabilities/` (core, beside the
...
- `bun test packages/runtime/src/hooks/registry.test.ts` pins the primitive's decoupling: `runHook` drives a stub handler to each `HookOutcome` (with the `payload` merge) importing NO `packages/runtime/src/jobs` symbol (used-independently), prototype-key rejection, a structural assertion that `packages/runtime/src/hooks/{types,registry,runner,index}.ts` import no `packages/runtime/src/jobs`/`packages/runtime/src/state`/`packages/runtime/src/integrations`/`packages/runtime/src/capabilities` module (the module boundary), and registration reachability (importing `builtins` makes `isKnownHook("skill-script")` true).
