...
1. `INSTRUCTIONS.md` content (falls back to the bundled `packages/runtime/src/runtime/defaults/INSTRUCTIONS.md` when the per-instance file is absent)
...
- `packages/runtime/src/execution/chat-task.ts` (modern agent loop) and `packages/runtime/src/provider.ts::generateTaskSummary` (legacy single-shot path) load the three files via `identity-files.ts` and pass them through `buildAgentSystemContext`.
- `packages/runtime/src/execution/tool-catalog.ts` adds `edit_soul` and `edit_user_profile` tools (toolset `identity`, always exposed). Both auto-approve clean bodies — writes land directly at `SOUL.md` / `USER.md` and are effective on the next system prompt — with the injection scan still gating threat patterns (hostile bodies route through `SOUL.md.proposed` / `USER.md.proposed` and stay out of the prompt until the user approves).
- `packages/runtime/src/execution/tool-dispatch.ts` routes `edit_soul` / `edit_user_profile` to handlers that call into `identity-files.ts`. The handlers are sync (no approval gate at dispatch time) and rely on the proposed-vs-approved file split to keep unreviewed content out of the prompt.
...
- **INSTRUCTIONS.md is user-only.** The agent has no tool to edit it. The bundled `packages/runtime/src/runtime/defaults/INSTRUCTIONS.md` remains shipped with the runtime so a fresh instance has a working preamble without filesystem setup.
- **Scaffold asymmetry.** At instance creation `install()` seeds `INSTRUCTIONS.md` with the bytes of the bundled `packages/runtime/src/runtime/defaults/INSTRUCTIONS.md` so the user opens the file to a working baseline they can edit against — an empty file gives them nothing to anchor on. `USER.md` stays zero-byte because no default exists (a user profile is inherently caller-supplied). Per-agent `SOUL.md` is seeded with `Your name is <name>.` (from `AgentRecord.name`) so a new agent self-identifies by its own name; the rest of the persona is caller-supplied. The seed never clobbers a populated SOUL, so a user/agent-authored body is preserved. Drift cost: the seeded `INSTRUCTIONS.md` shadows the bundled file, so without intervention an unedited instance copy would be frozen at install-time defaults even as the bundled file evolves on later Gini upgrades. The boot-time reseed below closes that gap; deletion remains a manual escape hatch — removing the file restores the bundled fallback path at the next chat turn.
- **Boot-time reseed of an unedited `INSTRUCTIONS.md`.** `install()` calls `reseedDefaultInstructions` on every boot (after the identity-line migration): when the instance file's sha256 digest matches a known previously-shipped bundled default — `HISTORICAL_DEFAULT_INSTRUCTIONS_HASHES` in `packages/runtime/src/runtime/identity-files.ts`, derived from the file's main-branch git history and including the variant each legacy default takes after `migrateInstructionsIdentityLine` rewrites its first identity line — the file is overwritten with the current bundled default, so defaults improvements reach existing installs. The never-clobber guarantee is structural: any user edit changes the digest, which then matches no shipped default, and the file is left untouched. A file already byte-identical to the current default is a no-op (no rewrite churn). The hash list is a checked-in constant regenerated when the bundled default changes (procedure in the source comment); the runtime never shells out to git.
...
- A fresh instance with no per-instance identity files present produces a system prompt that contains the trimmed bytes of the bundled `packages/runtime/src/runtime/defaults/INSTRUCTIONS.md` and no SOUL/USER blocks.
...
- A `gini install` against a fresh instance copies the bytes of `packages/runtime/src/runtime/defaults/INSTRUCTIONS.md` into `~/.gini/instances/<inst>/INSTRUCTIONS.md` byte-for-byte (the scaffold and runtime fallback never drift).
- At boot, an unedited `INSTRUCTIONS.md` whose sha256 matches a previously shipped default (per `HISTORICAL_DEFAULT_INSTRUCTIONS_HASHES`) is overwritten with the current bundled default, so a user who never edited the file tracks upgrades instead of freezing at install-time bytes; a file one byte off any shipped default (a user edit) is left untouched, and a file already equal to the current default is a no-op.
- If `packages/runtime/src/runtime/defaults/INSTRUCTIONS.md` is removed from the bundle, both `getDefaultGiniInstructions()` and `scaffoldInstanceIdentityFiles()` throw with a clear "default INSTRUCTIONS.md missing from bundle" message rather than silently falling back to an empty preamble.
...
- `packages/runtime/src/runtime/defaults/INSTRUCTIONS.md` — the canonical default operating rules. Single source of truth: the runtime fallback reads it (trimmed, memoized) and the scaffolder copies it bytes-as-is into freshly-installed instances.
- `packages/runtime/src/system-prompt.ts` — `getDefaultGiniInstructions()` + `DEFAULT_INSTRUCTIONS_PATH` + `sanitizeAgentName()`; `buildAgentSystemContext` accepts `instructionsOverride`, `soul`, `userProfile`.
- `packages/runtime/src/runtime/identity-files.ts` — load/write/scan helpers; `scaffoldInstanceIdentityFiles` reads the bundled defaults file once per call; `seedAgentSoulFile` seeds the per-agent name into `SOUL.md`.
- `packages/runtime/src/capabilities/agents.ts` (`createAgent`), `packages/runtime/src/runtime/index.ts` (`install()`) — seed/backfill the per-agent SOUL name.
- `packages/runtime/src/execution/chat-task.ts`, `packages/runtime/src/provider.ts` — call sites that load and forward the three files.
- `packages/runtime/src/execution/tool-catalog.ts`, `packages/runtime/src/execution/tool-dispatch.ts` — `edit_soul` and `edit_user_profile`.
