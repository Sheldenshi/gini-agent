...
3. **It is weakest exactly where we need the most reliability.** Headed connect for user sign-in and CDP attach to the user's own Chrome had open bugs at evaluation; the CDP-attach issue under Bun (playwright-core's bundled `ws` deadlocking the websocket handshake) is fixed in-place by `patches/playwright-core@1.60.0.patch` — a far smaller intervention than swapping the engine for one with the same class of bug and no API contract.
