...
## Browser transport: spawn-only

The runtime drives exactly one transport: a per-instance branded Chrome it **spawns itself**. There is no external-attach or visible-window alternative — a browser always comes from the runtime launching its own Chrome.

`src/tools/chrome-launch.ts` (`launchSpawnedChrome`) launches a branded Chrome via `chromium.launchPersistentContext` over Playwright's pipe transport — `--headless=new` + the shared stealth args + a clean (non-`HeadlessChrome`) UA + the per-instance `--user-data-dir` + a free-picked `--remote-debugging-port`. The TCP debug port is an extra local endpoint the sign-in screencast bridge attaches to over raw CDP, **not** the automation transport: `connectOverCDP` over a TCP WebSocket hangs under playwright-core 1.60 + Bun, so the pipe is used for driving the page. On a machine with neither a branded Chrome nor Playwright's bundled Chromium on disk, the launch downloads Playwright's Chromium on demand (`src/tools/chrome-install.ts`) and retries — so "no Chrome installed" is a supported, self-provisioning case rather than a hard failure.

The launched handle installs into the single per-instance `shared` slot — one shared browser per instance (cookies bleed across tasks within the instance, per the explicit product decision). Teardown closes the persistent context and, only if that wedges, reaps the Chrome bound to the instance's own profile dir (never `killall`, never the user's own `:9222`). See [Browser Stealth Identity](browser-stealth-identity.md).

Sign-in at an auth wall is handled by streaming this same spawned headless Chrome into an in-chat screencast modal (`browser_connect`; see [Browser Connect Handoff](browser-connect-handoff.md)) — the user signs in on the live page and the agent continues. There is no visible managed window and no `/api/browser/connect` external-attach endpoint.

- **The in-process trust machinery runs above the transport.** Snapshot walking, secret redaction, SSRF/domain-policy gating, approvals, and audit all run in-process against the Playwright client driving the spawned Chrome. This is the same property that ruled out subprocess engines (the Rationale above): page content always passes through the in-process redaction/gating hooks before reaching the model.
...
- A future need to drive a browser the runtime did NOT spawn (e.g. a remote/cloud session for IP reputation) reintroduces a non-pipe transport, making a CDP-native engine worth the integration cost.
