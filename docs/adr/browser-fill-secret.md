...
- The gateway's `/api/setup-requests/<id>/complete` handler is the seam for all SetupRequest actions. New fill-style actions added later (e.g. `browser.fill_otp` with a TTL-bound code) follow the same pattern: extend `SetupRequestAction`, add a branch, share the same body shape and BFF route.
