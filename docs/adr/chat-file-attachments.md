...
- The derived JPEG is **cached on disk** next to the original at `<id>.vis-<limit>.jpg` (`readVisionVariant`/`writeVisionVariant` in `packages/runtime/src/state/uploads.ts`), so compression runs **once** even though images replay every turn. The original upload is never mutated.
...
`resolveProviderModality` (`packages/runtime/src/provider-capabilities.ts`) returns `{ vision, nativeDocs }` per provider × model, from a static record built from each provider's **API reference** (not product/app file-upload features, which are app-side text extraction). Current record:
