...
For runtime / agent changes (tools, dispatch, providers, memory, skills), "the affected surface" is a **real chat turn**, not a unit test. Start the gateway, create a session, send a message through the chat flow (`bun run gini chat new --instance <instance>` → `bun run gini chat send <session> "<prompt>" --instance <instance>`, which posts to `/api/chat/<id>/messages` — the same path the web UI uses), wait for the task to complete, and confirm from the task's `recentToolCalls` + summary that the agent selected the right tool and produced the right result. Unit tests verify the mechanism; the chat turn verifies the model actually reaches for it. Test against the worktree's own instance, never `default`.
